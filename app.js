...
// Inject Car Cards & Dropdown
const carGrid = document.getElementById('carGrid');
const selectedCar = document.getElementById('selectedCar');

function renderCars(list) {
  carGrid.innerHTML = '';
  selectedCar.innerHTML = '<option value="">-- Select Your Car --</option>';
  list.forEach(car => {
    // Grid Cards
    ...
    card.innerHTML = `
      ...
      <button onclick="selectCar('${car.brand} ${car.model} ${car.year} ${car.color}')">Select</button>
    `;
    carGrid.appendChild(card);

    // Dropdown
    const option = document.createElement('option');
    option.value = `${car.brand} ${car.model} ${car.year} ${car.color}`;
    option.textContent = `${car.brand} ${car.model} ${car.year} (${car.color})`;
...
function searchCar() {
  ...
  const filtered = cars.filter(car => car.brand.toLowerCase().includes(query) || car.model.toLowerCase().includes(query));
  renderCars(filtered);
}

// Car selection button updates form dropdown
...
function loginUser() {
  alert("Login feature is front-end demo only.");
}

function signupUser() {
  alert("Sign-up feature is front-end demo only.");
}
...
function submitForm(e) {
  ...
  const data = {
    ...
    contactPrimary: document.getElementById('contactPrimary').value,
    contactSecondary: document.getElementById('contactSecondary').value,
    nin: document.getElementById('nin').value,
    car: document.getElementById('selectedCar').value,
    deliveryDate: document.getElementById('deliveryDate').value,
    deliveryMethod: document.getElementById('deliveryMethod').value
  };
  console.log("Submitted Vehicle Registration:", data);
  document.getElementById('formSuccess').textContent = "Registration submitted successfully!";
